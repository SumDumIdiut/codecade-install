...
# True if something (tracked or not) is already listening on 127.0.0.1:$1.
# Uses bash's built-in /dev/tcp instead of ss/fuser/lsof, which aren't
# guaranteed present on the Windows dev machine this same script also runs
# on -- /dev/tcp works identically there under Git-Bash/MSYS.
#
# Exists because proc_running() only ever checks a service's OWN pidfile --
# it has no way to notice a port that's already bound by a process the
# pidfile doesn't currently point at (confirmed live on terraserver: a
# start_forge() attempt raced the still-running previous git-forge process,
# EADDRINUSE'd and crashed immediately, and the crash's own PID got written
# into forge.pid -- orphaning the real, healthy, multi-day-uptime process
# from status-tracking and leaving every subsequent restart attempt to
# crash the exact same way, each one logging another identical stack trace
# to error.log). Callers should treat a positive result as "don't spawn a
# second instance that's guaranteed to crash," not as proof of which
# process owns it.
port_listening() {
...
# Kills whatever's already bound to 127.0.0.1:$1 (tracked or not) before a
# start_*() tries to listen there. Linux/macOS only.
free_port() {
  local port="$1" os; os="$(detect_os)"
  { [ "$os" = "linux" ] || [ "$os" = "darwin" ]; } || return 0
  port_listening "$port" || return 0
  local pids=""
  if command -v fuser >/dev/null 2>&1; then
    pids=$(fuser -n tcp "$port" 2>/dev/null | tr -d '\r')
  fi
  ...
  [ -z "$pids" ] && return 0
  warn "port $port is already held by a leftover process (PID $pids) — killing it before starting."
  local pid
...
const DOTNET_RELAY_TARGET = process.env.DOTNET_RELAY_TARGET || 'http://127.0.0.1:7777';
const RECHARGE_HUB_TARGET = process.env.RECHARGE_HUB_TARGET || 'http://127.0.0.1:3010';
...
// expects it.
// All proxies get their WS upgrades from the SAME explicit,
...
// Plain REST, no WebSockets at all - same shape as forgeProxy/tagRelayProxy,
// nothing for the ws:true footgun described above to catch here. Unlike
// temutalk/forge/tag, recharge-hub isn't BASE_PATH-aware (its own routes are
// plain /api/... with no mount-prefix handling), so the /recharge prefix
// needs to actually be stripped before forwarding, not just preserved.
const rechargeHubProxy = createProxyMiddleware('/recharge', {
  target: RECHARGE_HUB_TARGET,
  changeOrigin: true,
  pathRewrite: { '^/recharge': '' },
  logLevel: 'warn',
});
...
app.use('/recharge', rechargeHubProxy);
...
// Same raw-pipe approach as proxyTagWebSocket above, for the same reason:
// dotnet-relay is WS-only (no plain HTTP routes at all), so there's no
// Express mount for it to fall back on if this went through
// createProxyMiddleware's WS handling instead - and that path is exactly
// what broke /temutalk, /panel, and /chaos three separate times (see the
// long comment above temutalkProxy). A pure byte-for-byte splice has no
// WS-frame reparsing to get wrong.
function proxyDotnetWebSocket(req, socket, head) {
  socket.setTimeout(0);
  socket.setNoDelay(true);
  const target = new URL(DOTNET_RELAY_TARGET);
  const targetSocket = net.connect(target.port || 80, target.hostname, () => {
    targetSocket.setTimeout(0);
    targetSocket.setNoDelay(true);
    let raw = `${req.method} ${req.url} HTTP/1.1\r\n`;
...
server.on('upgrade', (req, socket, head) => {
  ...
  if (req.url.startsWith('/dotnet')) return proxyDotnetWebSocket(req, socket, head);
...
server.listen(PORT, () => {
  ...
  console.log(`    /dotnet   -> ${DOTNET_RELAY_TARGET}`);
...
<title>Oliver Prakke — Full-Stack Developer</title>
<meta name="description" content="Oliver Prakke — full-stack developer. Self-hosted tools and games: TemuTalk, Forge, Tag, Recharge, DOTnet.">
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Crect width='32' height='32' fill='%2305070f'/%3E%3Ccircle cx='4' cy='6' r='0.7' fill='%23fff'/%3E%3Ccircle cx='27' cy='24' r='0.9' fill='%23fff'/%3E%3Ccircle cx='26' cy='5' r='0.6' fill='%23fff'/%3E%3Ccircle cx='6' cy='26' r='0.6' fill='%23fff'/%3E%3Ccircle cx='16' cy='16' r='6.5' fill='%23091226'/%3E%3Ccircle cx='16' cy='16' r='6.5' fill='none' stroke='%238fe1ff' stroke-width='1.4'/%3E%3Cellipse cx='16' cy='16' rx='13' ry='3.6' fill='none' stroke='%238fe1ff' stroke-width='1.2' transform='rotate(-18 16 16)'/%3E%3C/svg%3E">
...
<style>
  ...
  * {
  ...
  /* ---- custom cursor: a small ring that follows the pointer, growing over
     interactive elements. Desktop/mouse only. ---- */
  .cursor-ring {
    ...
    transition: transform .12s ease-out, width .15s ease, height .15s ease, margin .15s ease, opacity .2s ease, background .15s ease;
  }
  .cursor-ring.active { opacity: .85; }
  .cursor-ring.hover {
    width: 34px; height: 34px; margin: -17px 0 0 -17px;
    background: rgba(143,225,255,.1); border-color: var(--bright);
  }
  @media (hover: none), (pointer: coarse) { .cursor-ring { display: none; } }
  @media (prefers-reduced-motion: reduce) { .cursor-ring { display: none; } }

  /* ---- deep-space backdrop: a plain starfield canvas, one faint nebula ---- */
  #stars { position: fixed; inset: 0; z-index: -3; display: block; background: var(--bg); }
  .nebula {
    position: fixed; z-index: -2; border-radius: 50%; pointer-events: none; will-change: transform;
    width: 55vmax; height: 55vmax; top: -22vmax; left: -18vmax;
    filter: blur(90px); opacity: .14;
    background: radial-gradient(circle, rgba(77,139,255,.8), transparent 65%);
    animation: driftA 60s ease-in-out infinite;
  }
  @keyframes driftA { 0%,100% { transform: translate(0,0); } 50% { transform: translate(3vmax,2vmax); } }
  @media (prefers-reduced-motion: reduce) { .nebula { animation: none !important; } }

  /* ---- corner glyphs: box-drawing corners instead of CSS-rounded buttons ---- */
  .bracketed { position: relative; }
  .bracketed::before, .bracketed::after {
    position: absolute; font-family: var(--font-mono); font-size: 15px; line-height: 1;
    color: var(--line-strong); pointer-events: none; z-index: 1;
  }
  .bracketed::before { content: '┌'; top: -1px; left: -1px; }
  .bracketed::after { content: '┘'; bottom: -1px; right: -1px; }

  /* ---- the four launcher buttons ---- */
  .launcher {
  ...
  .launch-btn {
    display: flex; align-items: center; justify-content: center;
    padding: 20px 22px; border: 1px solid var(--line); border-left-width: 3px;
    background: var(--panel); color: var(--bright); text-decoration: none;
    font-family: var(--font-display); font-weight: 800; font-size: 17px; letter-spacing: .01em;
    transition: background .15s ease, border-color .15s ease, transform .15s ease;
  }
  .launch-btn:hover { background: var(--panel-hover); transform: translateY(-2px); }
  .launch-btn.tt { border-left-color: var(--tt); }
  .launch-btn.fg { border-left-color: var(--fg); }
  .launch-btn.tc { border-left-color: var(--tc); }
  .launch-btn.rc { border-left-color: var(--rc); }
</style>
</head>
<body>
  ...
  <script>
  /* ---- plain starfield: two parallax layers, gentle twinkle, nothing else.
     Fully self-contained -- doesn't depend on anything else on the page. ---- */
  (function () {
    "use strict";
    ...
    var W = 0, H = 0, layers = [], running = true, t = 0;

    function buildLayers() {
      layers = [
        { speed: 0.006, size: [0.5, 1.1], twinkle: 0.3, density: 0.00007, stars: [] },
        { speed: 0.015, size: [0.9, 1.7], twinkle: 0.5, density: 0.00003, stars: [] },
      ];
      layers.forEach(function (layer) {
        var n = Math.max(20, Math.min(180, Math.round(W * H * layer.density)));
        for (var k = 0; k < n; k++) {
          layer.stars.push({
    ...
    function draw() {
      ...
      layers.forEach(function (layer) {
        var dy = t * layer.speed;
        layer.stars.forEach(function (s) {
          var yy = ((s.y + dy) % H + H) % H;
          var tw = reduceMotion ? 1 : 0.6 + Math.sin(t * 0.02 * layer.twinkle + s.phase) * 0.3;
          ctx.beginPath();
  ...
  <div class="launcher">
    <a class="launch-btn tt bracketed" href="/temutalk/">TemuTalk</a>
    <a class="launch-btn fg bracketed" href="/forge/">Forge</a>
    <a class="launch-btn tc bracketed" href="/tag/">Tag</a>
    <a class="launch-btn rc bracketed" href="/recharge/">Recharge</a>
  </div>
...
(function () {
  ...
  /* ---- custom cursor: a ring that follows the pointer and grows over
  ...
  if (!reduceMotion && window.matchMedia('(hover: hover)').matches) {
    ...
    if (cursorRing) {
      ...
      window.addEventListener('mousemove', function (e) {
        ...
        if (!shown) { cursorRing.classList.add('active'); shown = true; }
        cursorRing.classList.toggle('hover', !!e.target.closest('a, button'));
      }, { passive: true });
...
#tt-panes{flex:1;overflow:hidden;display:flex;flex-direction:column}
...
do_setup() {
  ...
  if [ -d "$DIR/recharge-hub" ]; then
    info "Installing recharge-hub dependencies..."
    run_capturing "recharge-hub-npm-install" bash -c "cd '$DIR/recharge-hub' && '$npm_bin' install --no-audit --no-fund --no-bin-links --loglevel=error" \
      && ok "recharge-hub dependencies installed."
  else
    warn "recharge-hub not found — skipping."
  fi
...
start_recharge_hub() {
  if proc_running recharge-hub; then warn "recharge-hub already running."; return; fi
  free_port "$RECHARGE_HUB_PORT"
  if [ ! -d "$DIR/recharge-hub/node_modules" ]; then warn "recharge-hub/node_modules missing — run setup first."; return; fi
  local node_bin; node_bin=$(find_node)
  if [ -z "$node_bin" ]; then err "node not found on PATH."; return; fi
...
status_json() {
  ...
  proc_running dotnet-relay  && dotnet_relay_run=true
...
do_check_updates() {
  ...
  if [ "$changed_temutalk" -eq 0 ] && [ "$changed_forge" -eq 0 ] && [ "$changed_tag" -eq 0 ] && [ "$changed_dotnet" -eq 0 ]; then
...
if [ "${1:-}" = "start" ] || [ "${1:-}" = "stop" ]; then
  ...
  case "$1-$2" in
    ...
    start-dotnet-relay)  start_dotnet_relay ;;
    ...
    stop-dotnet-relay)   stop_proc dotnet-relay ;;
...
menu() {
  while true; do
    ...
    print_status_row "DOTnet"    dotnet-relay
